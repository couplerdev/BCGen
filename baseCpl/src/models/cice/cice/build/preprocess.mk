...
#----------------------------------------------------------------------------
#
#  This makefile is called from the CICE driver makefile and performs
...
ifneq (,$(CICEARCH))
  ...
  bogus:
	@echo "  Please set CICEARCH environment variable"
endif
...
SRCDIRS = $(CICEEXEDIR)/
SRCDIRS := $(SRCDIRS) $(CICEDIR)/src/drivers/cpl_share/
SRCDIRS := $(SRCDIRS) $(CICEDIR)/src/drivers/cpl_mct/
SRCDIRS := $(SRCDIRS) $(CICEDIR)/src/mpi/
SRCDIRS := $(SRCDIRS) $(CICEDIR)/src/source/
...
ifneq (,$(FSRCS))
  SOURCES := $(addprefix $(CICEEXEDIR)/compile/, $(notdir $(FSRCS:.F=.f))) \
             $(SOURCES)
endif
...
ifneq (,$(F90SRCS))
  SOURCES := $(addprefix $(CICEEXEDIR)/compile/, $(notdir $(F90SRCS:.F90=.f90))) \
             $(SOURCES)
endif
...
ifneq (,$(CSRCS))
  SOURCES := $(addprefix $(CICEEXEDIR)/compile/, $(notdir $(CSRCS:.C=.c))) \
             $(SOURCES)
endif
...
ifneq (,$(LFSRCS))
  ...
  ifneq (,$(LFSRCS))
    SOURCES := $(addprefix $(CICEEXEDIR)/compile/, $(notdir $(LFSRCS))) \
               $(SOURCES)
  endif
endif
...
ifneq (,$(LF90SRCS))
  ...
  ifneq (,$(LF90SRCS))
    SOURCES := $(addprefix $(CICEEXEDIR)/compile/, $(notdir $(LF90SRCS))) \
               $(SOURCES)
  endif
endif
...
ifneq (,$(LCSRCS))
  ...
  ifneq (,$(LCSRCS))
    SOURCES := $(addprefix $(CICEEXEDIR)/compile/, $(notdir $(LCSRCS))) \
               $(SOURCES)
  endif
endif
...
$(CICEEXEDIR)/compile/%.f: %.F
	@echo '$(CICEARCH) preprocessing ' $<
	@$(Cpp) $(Cpp_opts) $< > $(CICEEXEDIR)/compile/$*.f

$(CICEEXEDIR)/compile/%.f90: %.F90
	@echo '$(CICEARCH) preprocessing ' $<
	@$(Cpp) $(Cpp_opts) $< > $(CICEEXEDIR)/compile/$*.f90

#  For some reason, our current Cpp options are incorrect for C files
#  so let the C compiler take care of ifdefs and just copy.

$(CICEEXEDIR)/compile/%.c: %.C
	@echo '$(CICEARCH) preprocessing ' $<
	@$(Cp) $< $(CICEEXEDIR)/compile/$*.c
...
$(CICEEXEDIR)/compile/%.f: %.f
	@echo '$(CICEARCH) preprocessing ' $<
	@$(Cp) $< $(CICEEXEDIR)/compile/$*.f

$(CICEEXEDIR)/compile/%.f90: %.f90
	@echo '$(CICEARCH) preprocessing ' $<
	@$(Cp) $< $(CICEEXEDIR)/compile/$*.f90
