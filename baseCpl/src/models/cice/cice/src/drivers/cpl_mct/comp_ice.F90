...
!
! !INTERFACE:
  subroutine ice_init_mct( compInfo, EClock, x2ice, ice2x, ierr )
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    ...
    type(mct_aVect)          , intent(inout) :: x2ice, ice2x
...
!-----------------------------------------------------------------------
    ...
    call time_clockGetInfo(EClock, dtime=dtime, calendar=calendar_type)
    dt = real(dtime)
    ...
    if (trim(gridcpl_file) == 'unknown_gridcpl_file') then
       call ice_SetGSMap_mct( MPI_COMM_ICE, ICE_ID, GSMap_ice ) 
       ...
       call ice_SetGSMap_mct( MPI_COMM_ICE, ICE_ID, GSMap_iloc ) 
       lsize_loc = mct_gsMap_lsize(gsMap_iloc, MPI_COMM_ICE)
       call ice_domain_mct( lsize_loc, gsMap_iloc, dom_iloc )
 
       call ice_setcoupling_mct(MPI_COMM_ICE, ICE_ID, gsmap_ice, dom_i)
       lsize = mct_gsMap_lsize(gsMap_ice, MPI_COMM_ICE)
 
       call ice_coffset_mct(xoff,yoff,gsmap_iloc,dom_iloc,gsmap_ice,dom_i,MPI_COMM_ICE)
 
       call ice_SetGSMap_mct( MPI_COMM_ICE, ICE_ID, gsmap_extend, xoff, yoff, nxcpl, nycpl)
       ...
       call mct_aVect_init(x2i_iloc, rList=trim(metaData%flds_x2ice), lsize=lsize_loc)
       call mct_aVect_zero(x2i_iloc)
       call mct_aVect_init(i2x_iloc, rList=trim(metaData%flds_ice2x), lsize=lsize_loc)
    ...
    call mct_aVect_init(x2ice, rList=trim(metaData%flds_x2ice), lsize=lsize)
    call mct_aVect_zero(x2ice)

    call mct_aVect_init(ice2x, rList=trim(metaData%flds_ice2x), lsize=lsize) 
    call mct_aVect_zero(ice2x)
    ...
    if (other_cplgrid) then
       call ice_prescribed_init(ICE_ID, gsmap_iloc, dom_iloc)
    else
       call ice_prescribed_init(ICE_ID, gsmap_ice, dom_i)
    endif
    ...
    if (other_cplgrid) then
       ...
       call ice_setdef_mct ( ice2x )
       call mct_rearr_rearrange(i2x_iloc, ice2x, rearr_iloc2ice)
    else
...
!
! !INTERFACE:
  subroutine ice_run_mct(compInfo, EClock, x2ice, ice2x, ierr)
!
! !DESCRIPTION:
...
! !ARGUMENTS:
    type(compMeta), target, intent(inout) :: compInfo
    type(ESMF_Clock),intent(in)    :: EClock
    type(mct_aVect), intent(inout) :: x2ice
    type(mct_aVect), intent(inout) :: ice2x
    integer, intent(inout) :: ierr
...
!---------------------------------------------------------------------------
    ...
    if (other_cplgrid) then
       call mct_rearr_rearrange(x2ice, x2i_iloc, rearr_ice2iloc)
       call ice_import_mct( x2i_iloc )
    else
       call ice_import_mct( x2ice )
    endif
...
!       call dumpfile(fname)
    ...
    stop_now = time_alarmIsOn( EClock , trim(alarm_stop_name))
    if (stop_now) then
...
!=======================================================================
  subroutine ice_setcoupling_mct(mpicom_i, ICE_ID, gsmap_i, dom_i)

    implicit none
    include 'netcdf.inc'

    integer        , intent(in)    :: mpicom_i
    integer        , intent(in)    :: ICE_ID
    ...
    call mct_gsmap_init(gsmap_i,npes,start,length,pe_loc,0,mpicom_i,ICE_ID,gsize)
