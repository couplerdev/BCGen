...
!--------------------------------------------------------------------------
  ...
  public flux_init_mct
  public flux_initexch_mct
  public flux_ocnalb_mct
  public flux_atmocn_mct
  public flux_atmocnexch_mct
...
!===============================================================================
  ...
  subroutine flux_init_mct( gsmap,dom, mpicom, fractions)
    ...
    type(mct_aVect), intent(inout)  :: fractions
    !
    ! Local variables
    !
    integer(in)                     :: nloc
    ...
    character(*),parameter :: subName =   '(flux_init_mct) '
    !-----------------------------------------------------------------------

    ! Set cdata pointers
    !gsmap = comp%comp_gsmap
    !dom = comp%domain
    !mpicom = comp%comm
...
!    where (fractions%rAttr(kx,:) > 0.0_r8) mask = nint(rmask)
  ...
  end subroutine flux_init_mct

!===============================================================================

  subroutine flux_initexch_mct(comp_a,comp_o)

    !-----------------------------------------------------------------------
    !
    ! Arguments
    !
    type(compMeta), intent(in)  :: comp_a
    type(compMeta), intent(in)  :: comp_o
    !
    ! Local variables
    !
    ...
    dom_a = comp_a%domain
    gsmap_a = comp_a%comp_gsmap
    mpicom_a = comp_a%comm
    ID_a = comp_a%ID

    dom_o = comp_o%domain
    gsmap_o = comp_o%comp_gsmap
    mpicom_o = comp_o%comm
    ID_o = comp_o%ID
  ...
  end subroutine flux_initexch_mct

!===============================================================================

  subroutine flux_ocnalb_mct( comp_o, xao_o, fractions_o)

    !-----------------------------------------------------------------------
    !
    ! Arguments
    !
    type(compMeta), target, intent(in)     :: comp_o
...
!===============================================================================
  ...
  subroutine flux_atmocnexch_mct( metaData, comp_a, comp_o, a2x, o2x, xao_a, xao_o, fractions_a, fractions_o)
    ...
    !
    character(*),parameter :: subName =   '(flux_atmocnexch_mct) '
    !
    !-----------------------------------------------------------------------
    ...
    fldlist = trim(metaData%flds_xao_states)//":"//trim(metaData%flds_xao_fluxes)//":sumwt"
...
!tcx   wt = sMato2a%data%rAttr(kw,n)
  ...
  end subroutine flux_atmocnexch_mct

!===============================================================================

  subroutine flux_atmocn_mct( comp_a, a2x, o2x, xao)
    ...
    !call seq_infodata_GetData(infodata,flux_albav=flux_albav)
    flux_albav = .true. ! hard encoding in code
    ...
    !call seq_infodata_GetData(infodata, dead_comps=dead_comps)
    dead_comps = .false. ! hard encoding in code
...
!                         duu10n,ustar, re  , ssq, missval = 0.0_r8 )
  ...
  end subroutine flux_atmocn_mct
