...
! from /share/BCGen/develDir/BCGenV2.1/BCGen/baseCpl/src/depUtils/ParallelIO/tests/general/pio_decomp_fillval.F90.in
...
USE pio_tutil
  ...
  integer, dimension(VEC_LOCAL_SZ) :: wbuf, rbuf, exp_val
  ...
  wcompdof = VEC_LOCAL_SZ * pio_tf_world_rank_ + compdof_rel_disps
   ! pio_decomp_fillval.F90.in:31
  wbuf = wcompdof
  exp_val = wcompdof
  rbuf = 0
   ! pio_decomp_fillval.F90.in:35
...
END SUBROUTINE nc_write_1d_explicit_fval_PIO_int_integer__
...
USE pio_tutil
  ...
  integer, dimension(:), allocatable :: wbuf
  ...
  rbuf = 0
   ! pio_decomp_fillval.F90.in:145
  call PIO_initdecomp(pio_tf_iosystem_, PIO_int, dims, wcompdof, wiodesc)
  call PIO_initdecomp(pio_tf_iosystem_, PIO_int, dims, rcompdof, riodesc)
   ! pio_decomp_fillval.F90.in:148
  num_iotypes = 0
  call PIO_TF_Get_nc_iotypes(iotypes, iotype_descs, num_iotypes)
    ...
    IF (.NOT. (PIO_TF_Passert_((ierr) == PIO_NOERR, pio_tf_comm_))) THEN
      ...
      IF (pio_tf_world_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
           "Failed to define a dim : " // trim(filename),&
          ":", __FILE__, ":", __LINE__,&
          "(pio_decomp_fillval.F90.in:158)"
      END IF
      RETURN
    END IF
   ! pio_decomp_fillval.F90.in:159
    ierr = PIO_def_var(pio_file, 'PIO_TF_test_var', PIO_int, (/pio_dim/), pio_var)
  ...
  deallocate(wbuf)
  deallocate(wcompdof)
END SUBROUTINE nc_write_1d_implicit_fval_PIO_int_integer__
...
USE pio_tutil
  ...
  rcompdof = VEC_LOCAL_SZ * pio_tf_world_rank_ + compdof_rel_disps
  wcompdof = VEC_LOCAL_SZ * pio_tf_world_rank_ + compdof_rel_disps
   ! pio_decomp_fillval.F90.in:223
  wbuf = wcompdof
  exp_val = wcompdof
  rbuf = BUF_FILLVAL
   ! pio_decomp_fillval.F90.in:227
    ...
    IF (.NOT. (PIO_TF_Passert_((ierr) == PIO_NOERR, pio_tf_comm_))) THEN
      ...
      IF (pio_tf_world_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
           "Failed to define a dim : " // trim(filename),&
          ":", __FILE__, ":", __LINE__,&
          "(pio_decomp_fillval.F90.in:247)"
      END IF
      RETURN
    END IF
   ! pio_decomp_fillval.F90.in:248
    ierr = PIO_def_var(pio_file, 'PIO_TF_test_var', PIO_int, (/pio_dim/), pio_var)
  ...
  call PIO_freedecomp(pio_tf_iosystem_, riodesc)
  call PIO_freedecomp(pio_tf_iosystem_, wiodesc)
END SUBROUTINE nc_read_1d_explicit_fval_PIO_int_integer__
...
USE pio_tutil
  ...
  integer, dimension(:), allocatable :: rbuf, exp_val
  ...
  wcompdof = VEC_LOCAL_SZ * pio_tf_world_rank_ + compdof_rel_disps
  wbuf = wcompdof
   ! pio_decomp_fillval.F90.in:322
  rbuf = BUF_FILLVAL
  do i=1,rcompdof_sz
    exp_val(i) = wbuf(i)
  end do
   ! pio_decomp_fillval.F90.in:327
  call PIO_initdecomp(pio_tf_iosystem_, PIO_int, dims, wcompdof, wiodesc)
  call PIO_initdecomp(pio_tf_iosystem_, PIO_int, dims, rcompdof, riodesc)
  ...
  deallocate(rbuf)
  deallocate(rcompdof)
END SUBROUTINE nc_read_1d_implicit_fval_PIO_int_integer__
